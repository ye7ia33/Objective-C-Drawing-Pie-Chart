...
- (void)drawRect:(CGRect)rect {
	...
	if (_progress > 0.0f) {
      ...
      /*  NSTimer *t = [NSTimer scheduledTimerWithTimeInterval:.01
                                                      target:self
        ...
        */
		...
		CGFloat angle = DEGREES_TO_RADIANS((360.0f * currprogress) + _AngleOffset);

		CGPoint points[3] = {
			CGPointMake(center.x + radius * cosf(DEGREES_TO_RADIANS(_AngleOffset)),center.y+ radius * sinf(DEGREES_TO_RADIANS(_AngleOffset))),
			center,
		...
		CGContextAddArc(context, center.x, center.y, radius, DEGREES_TO_RADIANS(_AngleOffset), angle, false);
...
-(void) animateProgress:(NSTimer *) aTimer
{
    ...
    if (currprogress<_progress) currprogress+=_progress/10.0;
